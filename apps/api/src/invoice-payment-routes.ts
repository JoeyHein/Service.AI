 ...
 *     Creates a Stripe PaymentIntent on the corporate Stripe account
 *     (single-account hub model from CHR-08), stores
 *     stripe_payment_intent_id and a 32-byte base64url
 *     payment_link_token. Stripe Connect / per-branch payout splitting
 *     was removed by CHR-08; commission accrual happens out-of-band via
 *     commission_ledger.
 *
...
> {
  ...
  if (feScope && invoice.branchId !== feScope) return { kind: 'not_found' };
  const brRows = await tx
    .select()
    .from(branches)
    .where(eq(branches.id, invoice.branchId));
  const branch = brRows[0];
  if (!branch) return { kind: 'not_found' };
  ...
  return { kind: 'ok', invoice, branch, customer };
}
...
export function registerInvoicePaymentRoutes(
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      const outcome = await withScope(db, scope, async (tx): Promise<Outcome> => {
        ...
        // CHR-08 corporate-hub model: charges land on the single corporate
        // Stripe account configured via STRIPE_SECRET_KEY. No Connect,
        // no application_fee_amount, no per-branch routing.
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      const sendContext = {
        branchId: outcome.invoice.branchId,
