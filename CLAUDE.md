...
>
> **Phase 18 note (2026-05).** Phase 18 (`phase_quote_fulfillment`, QF-01..07) closes the back half of the money loop. Accepting a quote auto-creates an `unassigned` job (`jobs.quote_id`) via `ensureJobForAcceptedQuote` (shared by both accept paths). Completing the job (`POST /jobs/:id/transition` → `completed`) drafts the balance invoice (`invoices.quote_id`, one-per-quote) from `balance-invoice.ts`: quote lines + a negative "Deposit (paid)" credit, `total` = balance due. **Commission is credited once — at quote commit; the balance-invoice payment webhook skips `onInvoicePaid` for quote-linked invoices.** The public invoice pay page is now wired to a shared `StripeCardForm` (`apps/web/src/lib/StripeCardForm.tsx`, reused by the quote deposit form). Migration `0020_quote_fulfillment.sql`. Detailed reference: `docs/api/quote-fulfillment.md`.
